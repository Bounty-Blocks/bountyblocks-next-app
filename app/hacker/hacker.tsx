...
const INITIAL_BUGS: BugItem[] = [
	{
		...
		body:
			"Opening a PSD and exporting previews stores EXIF with user email in a publicly reachable CDN URL. Steps to reproduce ...",
	},
	{
		id: "h2",
		companyId: "adobe",
		...
		body:
			"Prompt history is queryable by unauthenticated users when using a specific preview endpoint ...",
	},
	{
		id: "h3",
		companyId: "adobe",
		...
		body:
			"Unsanitized attribute leads to DOM XSS in help widget ...",
	},
];
		...
		<div className="min-h-screen grid grid-cols-1 md:grid-cols-3">
			...
			<aside className="border-r border-black/10 dark:border-white/10 p-4 md:p-6 flex flex-col gap-4 md:gap-6 md:col-span-1">
					...
					<div className="relative">
						<select
							...
							onChange={(e) => {
								...
								const firstTrack = TRACKS.find((t) => t.companyId === newCompany)?.id;
				...
				<div className="flex-1 min-h-0 overflow-hidden">
					<h2 className="mt-2 mb-2 text-xs font-medium uppercase tracking-wide text-foreground/60">Bugs submitted</h2>
					<div className="h-full overflow-auto rounded-lg border border-black/10 dark:border-white/10 divide-y divide-black/5 dark:divide-white/5">
						{filteredBugs.map((b) => (
							...
							>
								<p className="font-medium text-sm md:text-[15px] truncate">{b.title}</p>
								<p className="mt-1 line-clamp-1 text-xs text-foreground/60">{b.body}</p>
							</button>
						))}
						...
						<div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div className="grid grid-cols-3 gap-3 rounded-xl border border-black/10 dark:border-white/10 p-4">
								<div className="rounded-lg bg-black/[.03] dark:bg-white/[.03] p-3">
									...
									<p className="text-lg font-semibold">{usd(currentTrack.poolTotalUsd)}</p>
								</div>
								<div className="rounded-lg bg-black/[.03] dark:bg-white/[.03] p-3">
									<p className="text-xs text-foreground/60">Total Awarded</p>
									<p className="text-lg font-semibold">{usd(currentTrack.awardedUsd)}</p>
								</div>
								<div className="rounded-lg bg-black/[.03] dark:bg-white/[.03] p-3">
							...
							<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
								<div className="rounded-xl ring-1 ring-inset ring-rose-500/20 bg-rose-500/10 p-4">
									...
									<p className="text-lg font-semibold text-rose-300">{usd(currentTrack.rewardsUsd.high)}</p>
								</div>
								<div className="rounded-xl ring-1 ring-inset ring-amber-500/20 bg-amber-500/10 p-4">
									...
									<p className="text-lg font-semibold text-amber-300">{usd(currentTrack.rewardsUsd.medium)}</p>
								</div>
								<div className="rounded-xl ring-1 ring-inset ring-sky-500/20 bg-sky-500/10 p-4">
									...
									<p className="text-lg font-semibold text-sky-300">{usd(currentTrack.rewardsUsd.low)}</p>
								</div>
								<div className="rounded-xl ring-1 ring-inset ring-emerald-500/20 bg-emerald-500/10 p-4">
									...
									<p className="text-lg font-semibold text-emerald-300">{usd(currentTrack.rewardsUsd.minor)}</p>
								</div>
							</div>
						</div>

						<div className="prose prose-slate dark:prose-invert max-w-none mt-8">
							<h3 className="text-base font-semibold text-foreground/80">Track details</h3>
							<p className="mt-3 text-[15px] md:text-base">{currentTrack.description}</p>
						</div>
					</section>
				)}
				...
				{mode === "bug" && selectedBug && (
					<article className="mx-auto max-w-4xl">
						<header className="-mx-4 md:mx-0 border-b border-black/10 dark:border-white/10 px-4 md:px-0 pb-4">
							<h2 className="text-2xl md:text-3xl font-semibold leading-tight">{selectedBug.title}</h2>
						</header>
						<div className="relative mt-6">
							<div className="filter blur-sm select-none pointer-events-none">
								<p className="whitespace-pre-wrap leading-relaxed text-[15px] md:text-base">{selectedBug.body.repeat(4)}</p>
							</div>
							<div className="absolute inset-0 grid place-items-center">
								<div className="rounded-xl bg-background/90 backdrop-blur px-6 py-4 ring-1 ring-black/10 dark:ring-white/10 text-center">
									...
									<p className="text-xs text-foreground/60 mt-1">Only visible to sponsor triagers. Submit your own report to participate.</p>
								</div>
							</div>
						</div>
					</article>
				...
				{mode === "submit" && (
					<section className="mx-auto max-w-3xl">
						<header className="mb-6">
							...
							<p className="text-sm text-foreground/60 mt-1">Provide a concise summary and a detailed write-up.</p>
						</header>
						<form
							onSubmit={(e) => {
								e.preventDefault();
						...
						>
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								<div className="grid gap-2">
									...
									<select name="company" defaultValue={companyId} className="rounded-lg border border-black/10 dark:border-white/15 bg-background px-3 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-black/10 dark:focus:ring-white/10">
										{COMPANIES.map((c) => (
								...
								<div className="grid gap-2">
									...
									<select name="track" defaultValue={currentTrack?.id} className="rounded-lg border border-black/10 dark:border-white/15 bg-background px-3 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-black/10 dark:focus:ring-white/10">
										{companyTracks.map((t) => (
							...
							<div className="grid gap-2">
								...
								<input required name="title" placeholder="Summarize the bug in one sentence" className="rounded-lg border border-black/10 dark:border-white/15 bg-background px-3 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-black/10 dark:focus:ring-white/10" />
							</div>
							...
							<div className="grid gap-2">
								...
								<input type="email" name="email" placeholder="name@domain.com" className="rounded-lg border border-black/10 dark:border-white/15 bg-background px-3 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-black/10 dark:focus:ring-white/10" />
							</div>

							<div className="flex items-center gap-3">
								<button type="submit" className="rounded-lg bg-foreground text-background px-4 py-2 text-sm font-medium hover:opacity-90">Submit</button>
								<button type="button" onClick={() => setMode("track")} className="rounded-lg border border-black/10 dark:border-white/15 px-4 py-2 text-sm font-medium hover:bg-black/5 dark:hover:bg-white/5">Cancel</button>
							</div>
						</form>
					</section>
				)}
